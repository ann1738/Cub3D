...
/*   Updated: 2022/05/09 15:33:22 by anasr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_ray_dir_n_delta_dist(t_main *s)
{
	//calculate ray direction and initialize s->ray_map_position with the player map position
	s->ray_direction.x = cos(s->player_angle) + s->camera_plane.x;
	s->ray_direction.y = sin(s->player_angle) + s->camera_plane.y;
...
void	calc_inital_side_len(t_main *s)
{
	...
	if (s->ray_direction.y >= 0) //&& s->map[s->ray_map_position.y][s->ray_map_position.x] != '1')
	{
		s->step.y = 1;
		s->side_length_y = ((s->ray_map_position.y + 1) - s->player_position.y) * s->delta_distance_y;
	}
	else //if (s->map[s->ray_map_position.y][s->ray_map_position.x] != '1')
	{
		s->step.y = -1;
