...
/*   Updated: 2022/05/19 16:10:37 by anasr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int color)
{
	...
	+ (char)(fog_color->red * intensity)) & 255; // red
	g = ((char)((((color & 0x0000ff00) >> 8) & 255) * intensity_inverse) \
	+ (char)(fog_color->green * intensity)) & 255; // green
	b = ((char)((color & 0x000000ff) * intensity_inverse) \
	+ (char)(fog_color->blue * intensity)) & 255; // blue
	return (r << 16 | g << 8 | b);
}
